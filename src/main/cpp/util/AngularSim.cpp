...
AngularSim::AngularSim( double gearRatio, double sluggishness, const MotionConfig<units::turn> &mc )
      ...
      m_Profile{ {mc.mp.MaxVelocity, mc.mp.MaxAcceleration} }
{
    ...
    m_motorFF = new frc::SimpleMotorFeedforward<units::turn>{ 
    ...
    m_Goal = {MotionParams<units::turn>::Distance_t(0), MotionParams<units::turn>::Velocity_t(0) };
}
...
void AngularSim::SetMotionControl( units::turn_t goal ) 
{
    using_motion_control = true;
    m_Goal.position = goal; 
}

void AngularSim::SetPosition( units::turn_t position ) 
{ 
    m_motor.SetState( position, 0_rad_per_s ); 
}
...
void AngularSim::Update() 
{
    ...
    if( using_motion_control ) {
        ...
        units::turn_t currPosition = m_motor.GetAngularPosition();
...
const units::turn_t AngularSim::GetPosition()
{
    return m_motor.GetAngularPosition();
}

const units::revolutions_per_minute_t AngularSim::GetVelocity()
{
